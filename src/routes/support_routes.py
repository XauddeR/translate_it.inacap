...
def create_ticket():
    ...
    if not asunto or not mensaje:
        flash("Completa asunto y mensaje.", "ticket_error")
...
def view_ticket(ticket_id):
    ...
    cursor.execute("""
        SELECT id, autor_usuario_id, autor_admin_id, mensaje, creado_en
        FROM ticket_mensajes
        WHERE ticket_id = %s ORDER BY creado_en ASC
    """, (ticket_id,))
...
def reply_ticket(ticket_id):
    ...
    if not texto:
        flash("Escribe un mensaje.", "ticket_error")
        return redirect(url_for("support.view_ticket", ticket_id = ticket_id))
